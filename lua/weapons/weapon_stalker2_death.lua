...
function SWEP:PrimaryAttack()
	...
	elseif owner:GetNWFloat("DamageWasCrush") == true then  self:SendWeaponAnim(ACT_VM_PULLBACK)
		...
		if math.random(1,2) == 1 then self:SendWeaponAnim(ACT_VM_HITCENTER2) else self:SendWeaponAnim(ACT_VM_HITCENTER) end
	end
...
function SWEP:Think()
	...
	if (owner:KeyDown(IN_ATTACK) or owner:KeyDown(IN_ATTACK2) or owner:KeyDown(IN_JUMP) or owner:KeyDown(IN_FORWARD) or owner:KeyDown(IN_BACK) or owner:KeyDown(IN_MOVELEFT) or owner:KeyDown(IN_MOVERIGHT)) and SeqDur <= CurTime() then
		...
		owner:SetNWFloat("DamageWasGeneric",false)
		owner:SetNWFloat("DamageWasCrush",false)
		owner:SetNWFloat("DamageWasBullet",false)
		owner:SetNWFloat("DamageWasSlash",false)
		owner:SetNWFloat("DamageWasBurn",false)
		owner:SetNWFloat("DamageWasVehicle",false)
		owner:SetNWFloat("DamageWasBlast",false)
		owner:SetNWFloat("DamageWasElectrical",false)
		owner:SetNWFloat("DamageWasPoison",false)
		owner:SetNWFloat("DamageWasRadiation",false)
		owner:SetNWFloat("DamageWasDrown",false)
...
function SWEP:PostDrawViewModel(vm)
    ...
    if attachment then
        self.vmcameraPos = attachment.Pos
        self.vmcameraAng = attachment.Ang + Angle(0,-0,0)
    else
...
function SWEP:CalcView(ply, pos, ang, fov)
    if self.vmcameraPos then
        local offset = self.vmcameraPos - ply:EyePos()
        pos = pos + offset
    end
